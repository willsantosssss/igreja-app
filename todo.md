...
- [x] 🔴 BUG: Inscrições pagas em eventos - mostra 0 inscritos mas deveria mostrar dados - CORRIGIDO: 
  * Corrigidos nomes de colunas em getInscricoesEventosPagas (nomeInscrito, emailInscrito, telefoneinscrito, celulaInscrito)
  * Modificada createInscricaoEvento para criar automaticamente registro em pagamentos_eventos com status 'pendente'
...
## Bug: Botão "Deletar Conta" Não Funciona - Em Investigação
- [ ] Investigar por que requisição não chega ao servidor
- [ ] Verificar se perfil está dentro da árvore de providers
- [ ] Adicionar perfil como Stack.Screen no app/_layout.tsx
- [ ] Validar que tRPC client está sendo criado corretamente
- [ ] Testar com logs detalhados no cliente e servidor
- [ ] Corrigir autenticação/token se necessário
- [ ] Testar deletar conta com novo usuário

**Status:** Adicionado perfil como Stack.Screen no app/_layout.tsx. Requisição ainda não chega ao servidor. Próximo passo: verificar console do app para ver se há erros.
