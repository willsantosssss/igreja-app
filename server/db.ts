...
import { 
  ...
  contatosIgreja, InsertContatoIgreja, eventos, InsertEvento, noticias, InsertNoticia, avisoImportante, InsertAvisoImportante
} from "../drizzle/schema";
...
export async function updateContatosIgreja(data: { telefone: string; whatsapp: string; email: string }) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function updateAvisoImportante(data: { titulo: string; mensagem: string; ativo: number }) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  
  const existing = await db.select().from(avisoImportante).limit(1);
  if (existing.length > 0) {
    await db.update(avisoImportante).set(data).where(eq(avisoImportante.id, existing[0].id));
  } else {
